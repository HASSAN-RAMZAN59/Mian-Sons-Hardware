...
const mapCustomerAccountPayload = (customer) => ({
  id: customer.id,
  ...
  manualPaid: Number(customer.manualPaid || 0),
  createdAt: customer.createdDate || new Date().toISOString().split('T')[0]
});
...
const Customers = () => {
  ...
  const loadCustomersFromOrders = () => {
    ...
    const upsertCustomer = (key, data) => {
      if (!customersMap.has(key)) {
        customersMap.set(key, {
          ...
          manualPaid: Number(data.manualPaid || 0),
          totalPurchases: 0,
          totalPaid: 0,
          balanceDue: 0,
          notes: data.notes || '',
        ...
        customersMap.set(key, {
          ...
          openingBalance: existing.openingBalance || Number(data.openingBalance || 0),
          manualPaid: Number(existing.manualPaid || 0) + Number(data.manualPaid || 0),
    ...
    const customersList = Array.from(customersMap.values()).map((customer, index) => {
      const totalPaid = Number(customer.totalPaid || 0) + Number(customer.manualPaid || 0);
      const balanceDue = Math.max(0, customer.openingBalance + customer.totalPurchases - totalPaid);
  ...
  const handleAddCustomer = () => {
    ...
    writeStoredData(CUSTOMER_ACCOUNTS_KEY, updatedCustomers.map(mapCustomerAccountPayload));
    window.dispatchEvent(new CustomEvent('app-storage-updated', { detail: { key: CUSTOMER_ACCOUNTS_KEY } }));
  ...
  const handleProcessPayment = () => {
    ...
    const updatedCustomers = customers.map(c => 
      c.id === selectedCustomer.id 
        ? {
            ...c,
            manualPaid: Number(c.manualPaid || 0) + amount,
            totalPaid: Number(c.totalPaid || 0) + amount,
            balanceDue: Math.max(0, Number(c.balanceDue || 0) - amount)
          }
        : c
    );
