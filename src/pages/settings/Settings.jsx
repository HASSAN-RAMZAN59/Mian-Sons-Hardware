...
const Settings = () => {
  ...
  const STORE_LOGO_SRC = '/images/store-logo.png';

  // Store Information State
  const [storeInfo, setStoreInfo] = useState({
    storeName: 'Mian & Sons Hardware Store',
    logoUrl: STORE_LOGO_SRC,
  ...
  useEffect(() => {
    try {
      ...
      if (storedStoreInfo && typeof storedStoreInfo === 'object') {
        setStoreInfo((prev) => ({ ...prev, ...storedStoreInfo, logoUrl: STORE_LOGO_SRC }));
      }
    } catch (error) {
  ...
  useEffect(() => {
    try {
      ...
      if (storedInvoiceSettings && typeof storedInvoiceSettings === 'object') {
      ...
      if (storedSystemSettings && typeof storedSystemSettings === 'object') {
  ...
  const handleBackupDatabase = () => {
    ...
    localStorage.setItem(BACKUP_SNAPSHOT_KEY, JSON.stringify({
      createdAt: new Date().toISOString(),
      data: snapshot
    }));

    const nextBackupInfo = { ...backupInfo, lastBackupDate: new Date().toLocaleString() };
    setBackupInfo(nextBackupInfo);
    localStorage.setItem(BACKUP_INFO_KEY, JSON.stringify(nextBackupInfo));
    window.dispatchEvent(new CustomEvent('app-storage-updated', { detail: { key: BACKUP_INFO_KEY } }));
    window.dispatchEvent(new CustomEvent('app-storage-updated', { detail: { key: BACKUP_SNAPSHOT_KEY } }));
  ...
  const handleRestoreBackup = () => {
    if (window.confirm('Are you sure you want to restore from backup? Current data will be replaced.')) {
      try {
        const snapshotRaw = localStorage.getItem(BACKUP_SNAPSHOT_KEY);
        if (!snapshotRaw) {
          toast.error('No backup found. Please create a backup first.');
          return;
        }
        ...
        if (!snapshotData || typeof snapshotData !== 'object') {
          toast.error('Backup is invalid or corrupted.');
          return;
        }
        ...
        window.dispatchEvent(new CustomEvent('app-storage-updated', { detail: { key: null } }));
  ...
  const handleLogoUpload = (e) => {
    ...
    setStoreInfo({ ...storeInfo, logoUrl: STORE_LOGO_SRC });
