...
const Employees = () => {
  ...
  const handleGenerateSalarySlip = async (employee) => {
    ...
    try {
      const data = await employeeService.getSalarySlip(
        employee.id,
        salarySlipMonth,
        salarySlipYear
      );
          ...
          <div className="space-y-4">
            ...
            <Card>
              <div className="text-center mb-6">
                ...
                <p className="text-sm text-gray-500">{monthOptions.find((m) => m.value === salarySlipMonth)?.label} {salarySlipYear}</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                  <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Earnings</h4>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span>Basic Salary</span>
                      <span>Rs. {Number(salarySlip.slip?.basicSalary ?? salarySlip.employee.basicSalary ?? 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Allowances</span>
                      <span>Rs. {Number(salarySlip.slip?.allowances ?? salarySlip.employee.allowances ?? 0).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span>Rs. {Number(salarySlip.slip?.deductions ?? 0).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              </div>
