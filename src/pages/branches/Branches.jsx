...
const Branches = () => {
  ...
  const [branches, setBranches] = useState(() => {
    const storedBranches = readStoredData(BRANCHES_KEY);
    if (storedBranches.length) return storedBranches;
  ...
  const employees = readStoredData(EMPLOYEES_KEY).map((employee) => employee.name).filter(Boolean);

  useEffect(() => {
    localStorage.setItem(BRANCHES_KEY, JSON.stringify(branches));
    window.dispatchEvent(new CustomEvent('app-storage-updated', { detail: { key: BRANCHES_KEY } }));
  }, [branches]);
  ...
  const handleSubmit = (e) => {
    ...
    if (editingBranch) {
      ...
      const nextIndex = branches.length
        ? Math.max(...branches.map((branch) => Number(String(branch.id || '').replace('BR-', '')) || 0)) + 1
        : 1;

      const newBranch = {
        id: `BR-${String(nextIndex).padStart(3, '0')}`,
        ...formData,
        totalSales: 0,
        stockValue: 0,
        employeeCount: 0,
